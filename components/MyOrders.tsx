...
export const MyOrders: React.FC<MyOrdersProps> = ({ userLocation, onPayNow }) => {
  ...
  // Helper to calculate expected arrival time (Mock logic based on order date + 45 mins)
  ...
  const handleRatingSubmit = (rating: number, comment: string) => {
      console.log(`Rating submitted for order ${selectedRatingOrder?.id}: ${rating} stars, "${comment}"`);
  ...
  return (
    <div className="pb-32 px-4 space-y-4 pt-6">
      ...
      {orders.map((order, idx) => {
        // Auto-expand the first active order if none selected
                ...
                <div className="mt-4 pt-4 border-t border-slate-50 animate-fade-in">
                    {!isCancelled && !isCompleted && order.storeLocation && (
                        <div className="mb-4" onClick={(e) => e.stopPropagation()}>
                            <div className="h-40 rounded-xl overflow-hidden border border-slate-100 relative">
                                <MapVisualizer
                                    stores={[mapStore]}
                                ...
                                />
                                {isLive && (
                                    <div className="absolute top-2 left-2 bg-white/90 backdrop-blur px-2 py-1 rounded-lg text-[10px] font-black text-brand-dark shadow-sm z-[1000] border border-white/50 animate-pulse">
                                        ● Live Tracking
                                    </div>
                                )}
                            </div>
                            
                            {/* Explicit Navigation Button for Pickup */}
                            {order.mode === 'PICKUP' && (
                                <button 
                                    onClick={() => handleNavigate(order.storeLocation!.lat, order.storeLocation!.lng)}
                                    className="w-full mt-2 py-3 bg-blue-50 text-blue-700 rounded-xl font-bold text-xs flex items-center justify-center gap-2 hover:bg-blue-100 transition-colors"
                                >
                                    <span>📍</span> Get Directions to Store
                                </button>
                            )}
                        </div>
                    )}
                    ...
                    <div className="flex justify-between items-center mt-4 pt-2 border-t border-slate-100 mb-1">
                        <span className="font-bold text-xs text-slate-500">Total Paid</span>
                        <span className="font-black text-sm text-slate-900">₹{order.total}</span>
                    </div>
                    <div className="mb-3">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Via {order.paymentMethod || 'Online'}</span>
                    </div>
                    ...
                    {isCompleted && (
                        <div className="grid grid-cols-2 gap-2">
                            <button 
                                onClick={(e) => { e.stopPropagation(); setSelectedRatingOrder(order); }}
                                className="py-2.5 bg-slate-50 text-slate-700 rounded-xl font-bold text-xs hover:bg-slate-100 transition-colors"
                            >
                                ⭐ Rate Order
                            </button>
                            <button 
                                onClick={(e) => { e.stopPropagation(); setSelectedInvoiceOrder(order); }}
                                className="py-2.5 bg-slate-50 text-slate-700 rounded-xl font-bold text-xs hover:bg-slate-100 transition-colors flex items-center justify-center gap-1"
                            >
                                📄 Invoice
                            </button>
                        </div>
