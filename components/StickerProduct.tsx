...
export const StickerProduct: React.FC<StickerProductProps> = ({ product, onAdd, onUpdateQuantity, onClick, count }) => {
  ...
  return (
    ...
    >
      {/* Discount Badge */}
      {discount > 0 && (
         <div className="absolute top-0 left-0 bg-brand-DEFAULT text-white text-[9px] font-black px-2 py-1 rounded-br-xl z-20 shadow-sm">
             {discount}% OFF
         </div>
      )}
      ...
      <div className="relative aspect-square mb-3 rounded-2xl bg-slate-50 flex items-center justify-center group-hover:bg-brand-light/40 transition-colors">
        ...
        {count > 0 && (
          <div className="absolute top-2 right-2 bg-brand-DEFAULT text-white text-[10px] font-bold w-6 h-6 rounded-full flex items-center justify-center shadow-lg animate-scale-in">
            {count}
          </div>
        )}
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between mt-3 pt-2">
          <div className="flex flex-col">
              ...
              {mrp && mrp > price && (
                  <span className="text-[10px] text-slate-400 line-through font-medium">₹{mrp}</span>
              )}
          </div>
